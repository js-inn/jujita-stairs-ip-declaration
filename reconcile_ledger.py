...
def reconcile():
    ...
    # Rebalancing transfer to clear the ghost node and restore primary card balance
    block_content = {
        "index": new_index,
    ...
    """, (
        ...
        "VCARD-JUJITA-STAIRS-A9A55F37", 360.00, "CAD", nonce, block_hash, 
        "10839477 Canada Inc.", CREATOR_UUID
    ))
    conn.commit()
    conn.close()
    print(f"\n[SUCCESS] Ledger reconciled on Block #{new_index}! Ghost node balanced out.\n")
