...
def validate_schedule12_xml(xml_path, db_path="tax_audit_log.db"):
    # 1. Load Schedule 12 schema rules from database
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT field_name, min_occurs, max_occurs FROM schedule12_schema_fields")
    schema_rules = {row[0]: {"min": int(row[1]), "max": int(row[2])} for row in cursor.fetchall()}
    conn.close()

    if not schema_rules:
        print("Error: No Schedule 12 schema rules found in database. Run parse_schedule12.py first.")
        return False
    ...
    if errors:
        print("Schedule 12 Validation Failed:")
        for err in errors:
            print(f"  ❌ {err}")
        return False
    else:
        print(f"Validation Successful: '{xml_path}' complies with all T5013 Schedule 12 database schema rules.")
        return True

if __name__ == "__main__":
    validate_schedule12_xml("schedule12_submission.xml")
