...
def record_money_order():
    ...
    # Log postal money order issuance for $360.00 CAD
    block_content = {
        "index": new_index,
        ...
        "currency": "CAD",
        "nonce": nonce,
        "action": "POSTAL_MONEY_ORDER_ISSUE",
    ...
    """, (
        ...
        "CANADA-POST-EDMONTON-MAIN", 360.00, "CAD", nonce, block_hash, 
        "10839477 Canada Inc.", CREATOR_UUID
    ))
    conn.commit()
    conn.close()
    print(f"\n[SUCCESS] Block #{new_index} executed: $360.00 CAD Postal Money Order recorded!\n")
