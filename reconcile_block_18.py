...
def reconcile_final():
    ...
    # Transfer exact amount to zero out the negative ghost balance
    block_content = {
        "index": new_index,
        "previous_hash": prev_hash,
        "timestamp": timestamp,
        "sender_card": "VCARD-JUJITA-STAIRS-A9A55F37",
        "recipient_card": "VCARD-JUJITA-STAIRS-A9A55F37CANADA-POST-EDMONTON-MAIN",
        "amount": 1440.00,
        "currency": "CAD",
        "nonce": nonce,
        "action": "GHOST_NODE_ELIMINATION",
    ...
    """, (
        new_index, prev_hash, timestamp, "VCARD-JUJITA-STAIRS-A9A55F37", 
        "VCARD-JUJITA-STAIRS-A9A55F37CANADA-POST-EDMONTON-MAIN", 1440.00, "CAD", nonce, block_hash, 
        "10839477 Canada Inc.", CREATOR_UUID
    ))
    conn.commit()
    conn.close()
    print(f"\n[SUCCESS] Block #{new_index} executed: Ghost node completely neutralized!\n")
