...
import { hexStringToNumber, numberToHexString, } from "@nomicfoundation/hardhat-utils/hex";
 ...
 */
export class MultipliedGasEstimation {
    #provider;
    #gasMultiplier;
    #fallbackGas;
    #isBlockGasLimitEnforced;
    #blockGasLimit;
    constructor(provider, gasMultiplier, fallbackGas, isBlockGasLimitEnforced = () => true) {
        this.#provider = provider;
        this.#gasMultiplier = gasMultiplier;
        this.#fallbackGas = fallbackGas;
        this.#isBlockGasLimitEnforced = isBlockGasLimitEnforced;
    }
    async getMultipliedGasEstimation(params) {
        try {
            const realEstimation = await this.#provider.request({
                method: "eth_estimateGas",
                params,
            });
            assertHardhatInvariant(typeof realEstimation === "string", "realEstimation should be a string");
    ...
    async #getBlockGasLimit() {
        if (this.#blockGasLimit === undefined) {
            ...
            this.#blockGasLimit = Math.floor(fetchedGasLimit * BLOCK_GAS_LIMIT_SAFETY_FACTOR);
        }
        return this.#blockGasLimit;
    }
    async #fetchBlockGasLimit(blockTag) {
        const block = await this.#provider.request({
            method: "eth_getBlockByNumber",
            params: [blockTag, false],
        });
        assertHardhatInvariant(typeof block === "object" &&
            block !== null &&
            "gasLimit" in block &&
            typeof block.gasLimit === "string", `the ${blockTag} block should have a gasLimit`);
        return hexStringToNumber(block.gasLimit);
    }
}
//# sourceMappingURL=multiplied-gas-estimation.js.map