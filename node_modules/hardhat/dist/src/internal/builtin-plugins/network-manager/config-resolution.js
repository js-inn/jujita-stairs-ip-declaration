...
import { DEFAULT_EDR_NETWORK_HD_ACCOUNTS_CONFIG_PARAMS, EDR_NETWORK_DEFAULT_COINBASE, } from "./edr/edr-constants.js";
...
export function resolveEdrNetwork(networkConfig, defaultChainType, cachePath, resolveConfigurationVariable) {
    return {
        type: "edr-simulated",
        ...
        blockGasLimit: typeof networkConfig.blockGasLimit === "number" ||
        ...
        hardfork: resolveHardfork(networkConfig.hardfork, networkConfig.chainType ?? defaultChainType),
        ...
        transactionGasCap: typeof networkConfig.transactionGasCap === "number" ||
...
export async function resolveChainDescriptors(chainDescriptors) {
    ...
    for (const [chainId, userDescriptor] of Object.entries(chainDescriptors)) {
        ...
        const existingBlockExplorers = existingDescriptor.blockExplorers;
        for (const [explorerName, explorerConfig] of Object.entries(userDescriptor.blockExplorers ?? {})) {
            existingBlockExplorers[explorerName] = deepMerge(existingBlockExplorers[explorerName] ?? {}, explorerConfig, false);
        }
        resolvedChainDescriptors.set(toBigInt(chainId), existingDescriptor);
    }
    return resolvedChainDescriptors;
}
export function resolveHardfork(hardfork, chainType) {
