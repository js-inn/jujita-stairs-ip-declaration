...
export class HardhatHelpers {
    ...
    async getSigner(address) {
        const signerWithAddress = await HardhatEthersSigner.create(this.#provider, this.#networkName, this.#networkConfig, address);
    ...
    async getContractFactory(nameOrAbi, bytecodeOrFactoryOptions, signer) {
        if (typeof nameOrAbi === "string") {
            ...
            return await this.getContractFactoryFromArtifact(artifact, 
        ...
        return await this.#getContractFactoryByAbiAndBytecode(nameOrAbi, bytecodeOrFactoryOptions, signer);
    }
    async getContractFactoryFromArtifact(artifact, signerOrOptions) {
        ...
        if (artifact.bytecode === "0x") {
            throw new HardhatError(HardhatError.ERRORS.HARDHAT_ETHERS.GENERAL
                .INVALID_ABSTRACT_CONTRACT_FOR_FACTORY, { contractName: artifact.contractName });
        }
        const linkedBytecode = await this.#collectLibrariesAndLink(artifact, libraries);
        return await this.#getContractFactoryByAbiAndBytecode(artifact.abi, linkedBytecode, signer);
    }
    ...
    async #collectLibrariesAndLink(artifact, libraries) {
        ...
        const neededLibrariesByName = new Map();
        const neededLibrariesByFqn = new Map();
        ...
        for (const [linkedLibraryName, linkedLibraryAddress] of Object.entries(libraries)) {
            ...
            const matchingNeededLibraryByFqn = neededLibrariesByFqn.get(linkedLibraryName);
            const matchingNeededLibraries = matchingNeededLibraryByFqn !== undefined
