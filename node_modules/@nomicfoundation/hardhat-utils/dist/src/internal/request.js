...
export async function getBaseRequestOptions(requestUrl, { extraHeaders, abortSignal, queryParams } = {}, dispatcherOrDispatcherOptions) {
    ...
    const dispatcher = dispatcherOrDispatcherOptions instanceof undici.Dispatcher
    ...
    return {
        dispatcher: dispatcher.compose(undici.interceptors.redirect({ maxRedirections: DEFAULT_MAX_REDIRECTS }), 
 ...
 */
function describeResponseError(e) {
    ...
    e.message = `Response status code ${statusCode}${reason !== undefined ? `: ${reason}` : ""}`;
}
